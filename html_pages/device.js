...
function initChart() {
    ...
    setTimeout(getData,300); //add in slight delay before data is retrieved.
    //getData();
}

function getData() {
    $.ajax( {type:"Get", url:"device-step-telemetry.php",data: null, success: function(result) {
        drawStepChart(JSON.parse(result));
    } } );
    $.ajax( {type:"Get", url:"device-heartrate-telemetry.php",data: null, success: function(result) {
...
function drawHeartrateChart(data) {
    ...
    var options = {
        title: 'Heartrates',
        hAxis: {title: 'Date',  maxValue: now}, //minValue: prior,
